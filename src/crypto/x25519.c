...
#else /* fixed by chunghan.yi@gmail.com -- warning: ‘mul’ reading 32 bytes from a region of size 4 [-Wstringop-overread] */
static void ladder_part1(fe xs[5]) {
    limb_t *x2 = xs[0], *z2=xs[1],*x3=xs[2],*z3=xs[3],*t1=xs[4];
	limb_t a24_temp[NLIMBS] = {0,};
	a24_temp[0] = a24[0];
    add(t1,x2,z2);  // t1 = A
    sub(z2,x2,z2);  // z2 = B
    add(x2,x3,z3);  // x2 = C
    ...
    sqr1(z2);       // z2 = BB
    sub(x2,t1,z2);  // x2 = E = AA-BB
    mul(z2,x2,a24_temp,sizeof(a24)/sizeof(a24[0])); // z2 = E*a24
    add(z2,z2,t1);  // z2 = E*a24 + AA
}
#endif
